...
public class Data implements Comparable<Data> {
    ...
    public Data(int ano, int mes, int dia) {
        validarMes(mes); // valida o mês
        Mes mesEnum = Mes.obterMes(mes);
        validarDia(dia, mesEnum, ano); // valida o dia
        this.ano = ano;
        this.mes = mesEnum;
        this.dia = dia;
    }
    ...
    private void validarDia(int dia, Mes mes, int ano) {
        int diasNoMes = mes.numeroDeDias(ano);
        if (dia < 1 || dia > diasNoMes) {
            throw new DiaInvalidoException("Dia inválido: " + dia + " para o mês " + mes + " e ano " + ano);
        }
    }
